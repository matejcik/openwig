...
public class Zone extends Thing {
	...
	protected void setItem (String key, Object value) {
		if ("Points".equals(key) && value != null) {
			...
			for (int i = 1; i <= n; i++) {
				ZonePoint zp = (ZonePoint) lt.rawget(i);
				points[i-1] = zp;
			}
			if (active) {
				preprocess();
	...
	public int visibleThings() {
		...
		int count = 0;
		for (var o: inventory.getDelegate()) {
	...
	public void collectThings (List<Thing> c) {
		if (!showThings()) return;
		for (var entry: inventory.getDelegate()) {
			if (!(entry instanceof Thing)) continue;
			Thing t = (Thing)entry;
			if (t.isVisible()) c.add(t);
		}
	}
